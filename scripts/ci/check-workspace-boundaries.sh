...
check_absent frame-platform-lifecycle '^(gstreamer|glib|frame-[^[:space:]]+|worker|worker-sys|axum|leptos|reqwest|tokio) v'
...
if rg -n \
  ...
  --glob '!crates/windows-capture-ffi/**' \
  --glob '!crates/platform-lifecycle/**'; then
  printf 'unsafe Rust escaped the audited platform FFI crates\n' >&2
  fail=1
fi
