...
mod tests {
    ...
    fn topology_refresh_rebinds_only_regions_still_inside_the_same_display() {
        ...
        assert!(refresh_region_selections(&[display(2, 0)], &[region]) == vec![region]);
        assert!(refresh_region_selections(&[display(3, 0)], &[region]).is_empty());
        assert!(refresh_region_selections(&[display(2, 800)], &[region]).is_empty());
    }
