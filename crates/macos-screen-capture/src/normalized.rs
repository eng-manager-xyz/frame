...
impl MacOsNormalizedScreenCaptureSource {
    ...
    fn poll_control_or_topology(&mut self) -> Result<Option<NormalizedEvent>, ScreenSourceFailure> {
        ...
        let loss_reason = self
            ...
            .or_else(|error| {
                if error == MacOsCaptureError::StaleOrForeignTarget {
...
fn regions_from_snapshot(
    snapshot: &ScreenTargetSnapshot,
) -> Result<Vec<MacOsRegionSelection>, MacOsCaptureError> {
    snapshot
        .targets()
        .iter()
...
mod tests {
    ...
    fn normalized_capabilities_are_exact_and_protection_is_fail_closed() {
        ...
        assert!(capabilities.spec().topology_events);
        assert!(!capabilities.spec().target_recovery);
